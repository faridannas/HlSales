...
export default function NewTransactionPage() {
  ...
  const [nomorBon, setNomorBon] = useState(`BON-${Date.now()}`)
  ...
  useEffect(() => {
    if (selectedCustomer) {
      // Fetch bonus info for selected customer
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isBonusBon && bonusInfo && consumedBonus > bonusInfo.availableBonuses) {
      alert(`Jumlah klaim bonus melebihi bonus yang tersedia (${bonusInfo.availableBonuses}).`)
      return
    }
    ...
    if (res.ok) {
      ...
      alert(data.error || 'Failed to save transaction. Ensure the Invoice No is unique.')
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Create Invoice {isBonusBon && <span className="badge badge-bonus">BONUS</span>}</h1>
        </div>
        <button 
          type="button" 
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: '32px', borderTop: '1px solid var(--color-hairline)', paddingTop: '24px' }}>
            ...
            <button type="submit" className="btn-primary" style={{ width: '100%', padding: '16px', fontSize: '16px' }} disabled={loading || cart.length === 0 || !selectedCustomer}>
              {loading ? 'Saving...' : (isBonusBon ? 'Save BONUS Invoice (Paid)' : 'Save Invoice (Unpaid)')}
            </button>
          </div>
        </div>
